...
//
//    // Create a canny edge detector which will dynamically compute the threshold based on maximum
//    // edge intensity
//    // It has also been configured to save the trace as a graph.  This is the graph created while
//    // performing
...
//    // The 'edgeContours' is a tree graph that can be difficult to process.  An alternative is to
//    // extract
...
//  }
  ...
  private  <T extends ImageGray<T>, D extends ImageGray<D>> List<LineSegment2D_F32> detectLineSegments(
    ...
    DetectLineSegment<T> detector =
        FactoryDetectLine.lineRansac(new ConfigLineRansac(size / 40, threshold, 0.1, true), imageType);
  ...
  private void CalculateLines(List<LineSegment2D_F32> lines,BufferedImage srcImage) {
	  ...
	  if(showDebugImages) {		
	  BufferedImage linesImage=new BufferedImage(srcImage.getWidth(), srcImage.getHeight(), TYPE_INT_ARGB);
  ...
  private int calculationGap(List<VerticalHorizonLines> Lines) {
	// TODO Auto-generated method stub
      ...
      List<Float> needAvgGaps = new ArrayList<Float>();
      for (int s = 1; s < 5&&s< gapCounts.size(); s++)
      {
          if (Math.abs(gapCounts.get(s).gap - finalGap) < 3 &&gapCounts.get(s).counts > gapCounts.get(0).counts / 2)
          {
        	  needAvgGaps.add(gapCounts.get(s).gap);
          }
      }
      if (needAvgGaps.size() > 0)
      {
          int all = 0;
          for (int s = 0; s < needAvgGaps.size(); s++)
          {
              all +=  needAvgGaps.get(s);
          }
          return Math.round((finalGap+all) / (float)(needAvgGaps.size()+1));
      }
      else 
    	  return Math.round(finalGap);
}
...
public BoardPosition getBoardPosition(BufferedImage input) {
	 ...
	 if(horizonLines.size()<3||verticalLines.size()<3)
		 return null;
